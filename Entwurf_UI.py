...
# Saving the model
import pickle
pickle.dump(clf, open('immo_data_heute_clf.pkl', 'wb'))
...
if uploaded_file is not None:
    ...
    def user_input_features():
        immobilienart = st.sidebar.selectbox('immobilienart', ('Einfamilienhaus', 'Wohnung', 'Etagenwohnung', 'Sonstige', 'Mehrfamilienhaus', 'Erdgeschosswohnung', 'Erdgeschosswohnung', 'Dachgeschosswohnung', 'Zweifamilienhaus', 'Doppelhaushälfte', 'Villa', 'Reihenmittelhaus','Reihenendhaus', 'Bungalow', 'Maisonette', 'Apartment', 'Stadthaus', 'Schloss', 'Bauernhaus', 'Herrenhaus', 'Reiheneckhaus','Penthouse', 'Unbekannt'))
        ...
        angebotspreis = st.sidebar.slider('angebotspreis', 0, 200, 1000)
    ...
    st.write('Inputfeatures')
    st.write(input_df)
    
#-------------------kombination input features mit datensatz------------------------
...
df = df[:1] # Selects only the first row (the user input data)
    ...
    #dog1 = (input_df.dtypes == "object")
    #cat1 = input_df.columns[dog1].tolist()
    #df_enc = pd.get_dummies(input_df[cat1])
    #input_df_enc = pd.concat([input_df, df_enc], axis=1)
    #input_df_enc = input_df.drop(columns=cat1)
    
#df_enc = (input_df.dtypes == "object")
#input_df_catcol = input_df.columns[df_enc].tolist()
#input_df_enc = pd.get_dummies(input_df[input_df_catcol])
#input_df = pd.concat([input_df, input_df_enc], axis=1)
#input_df = input_df.drop(columns=input_df_catcol)
    
st.write('überführte Inputfeatures')
st.write(df)
...
#--------------model einlesen aus pickle datei--------------------------------------

load_clf = pickle.load(open('immo_data_heute_clf.pkl', 'rb'))

#--------------prediction auf das model anwenden------------------------------------

X_new = df.values
prediction = clf.predict(X_new)
#prediction_proba = clf.predict_proba(X_new)

#-------------darstellung der prediction-------------------------------------------
